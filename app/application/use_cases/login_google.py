...
class LoginGoogleUseCase:
    ...
    def execute(self, command: LoginGoogleInput) -> AuthTokensOutput:
        ...
        def _tx(auth_port: AuthPort) -> AuthTokensOutput:
            now = utcnow()
            identity = auth_port.get_identity_by_provider_subject(
                provider="google",
                provider_subject=google_identity.subject,
            )
            if identity is not None:
                user = auth_port.get_user_by_id(user_id=identity.user_id)
                if user is None:
                    raise ValueError("User linked to Google identity was not found.")
            else:
                user = auth_port.get_user_by_email(email=email)
                if user is None:
                ...
                google_identity_existing = auth_port.get_identity_for_user_provider(
                    user_id=user.id,
                    provider="google",
                )
                if google_identity_existing is None:
                    ...
                    auth_port.update_identity_provider_subject(
                        identity_id=google_identity_existing.id,
                        provider_subject=google_identity.subject,
                    )

            if google_identity.email_verified and not user.email_verified:
                auth_port.update_user_email_verified(user_id=user.id, email_verified=True)
                user = auth_port.get_user_by_id(user_id=user.id) or user
